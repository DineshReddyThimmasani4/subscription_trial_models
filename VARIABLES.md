...
## 2️⃣ **Post Engagement** (13 variables) ⚠️ **85% READY**

Dynamic point-in-time engagement metrics.

### Raw Counts (Stored)

| Variable | Status | Source | Location | Notes |
|----------|--------|--------|----------|-------|
| `views` | ✅ | PostActivityCount | line 454 | `lvc + gvc` |
| `shareCount` | ✅ | PostActivityCount | line 455 | `sc` |
| `mediaClickedCount` | ⚠️ | PostActivityCount | line 458 | **VERIFY**: using `vcc` field |
| `reactionCount` | ✅ | PostActivityCount | line 459 | `lrc + grc` |
| `commentCount` | ✅ | PostActivityCount | line 460 | `lcc + gcc` |
| `readModeCount` | ⚠️ | PostActivityCount | line 462 | **VERIFY**: using `rmc` field |
| `avgDur` | ✅ | PostActivityCount | line 463 | `ad` |
| `listenpct` | ✅ | PostActivityCount | line 464 | `lp` |
...
| `videoclicks_pv` | `mediaClickedCount / views` | views, mediaClickedCount | ⚠️ Depends on mediaClickedCount |
| `reactions_pv` | `reactionCount / views` | views, reactionCount | ✅ |
| `comments_pv` | `commentCount / views` | views, commentCount | ✅ |
| `seemore_pv` | `readModeCount / views` | views, readModeCount | ⚠️ Depends on readModeCount |

**Action Required:**
1. Query MongoDB: `db.PostActivityCount.findOne({}, {_id:0})`
2. Verify field names for:
   - Video/media clicks (currently using `vcc`)
   - Read mode/see more (currently using `rmc`)
3. Update lines 458 & 462 if field names differ
